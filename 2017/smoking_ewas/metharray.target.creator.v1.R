...
cat("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    METHYLATION ARRAY TARGET CREATOR -- FOR ATHERO-EXPRESS METHYLATION STUDY 1 & 2\n
    * Version: v1.2
    * Last edit: 2017-05-01
    * Created by: Sander W. van der Laan | s.w.vanderlaan-2@umcutrecht.nl\n
    * Description: Creates target files. Based on the DNAmArray workflow:
...
# opt$projectdir="~/PLINK/_AE_Originals"
...
cat("\n++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
cat("Starting \"Methylation Array Target Creator\".")
...
if(!is.na(opt$projectdir) & !is.na(opt$inputdir) & !is.na(opt$rawdata) & !is.na(opt$outputdir) & !is.na(opt$filename) & !is.na(opt$numberofcores)) {
  cat("\n* Setting directories and creating new ones.")
  ...
  cat("\n  - creating new (QC) directories where necessary...")
  ifelse(!dir.exists(file.path(OUT_loc)), 
         dir.create(file.path(OUT_loc)), 
         FALSE)
  
  ifelse(!dir.exists(file.path(INP_loc, "QC")), 
         dir.create(file.path(INP_loc, "QC"), mode = "0777"), 
         FALSE)
  QC_loc = paste0(INP_loc,"/QC")
  
  ifelse(!dir.exists(file.path(QC_loc, "Plots")), 
         dir.create(file.path(QC_loc, "Plots"), mode = "0777"), 
         FALSE)
  PLOT_loc = paste0(QC_loc,"/Plots")
  ifelse(!dir.exists(file.path(PLOT_loc, "Filter_Control")), 
         dir.create(file.path(PLOT_loc, "Filter_Control"), mode = "0777"), 
         FALSE)
  ifelse(!dir.exists(file.path(PLOT_loc, "Sample_Dependent")), 
         dir.create(file.path(PLOT_loc, "Sample_Dependent"), mode = "0777"), 
         FALSE)
  ifelse(!dir.exists(file.path(PLOT_loc, "Sample_Independent")), 
         dir.create(file.path(PLOT_loc, "Sample_Independent"), mode = "0777"), 
         FALSE)
  ...
  cat("\n* Reading in SampleSheets and creating targets...\n")
  targets <- read.metharray.sheet(RAW_loc)
  
  cat("\n* Write targets to a file...")
  base::save(targets, file = paste0(QC_loc,"/", opt$filename, ".targets.Rdata"))
...
#save.image(paste0(OUT_loc, "/",Today,".metharray.target.creator.RData"))
