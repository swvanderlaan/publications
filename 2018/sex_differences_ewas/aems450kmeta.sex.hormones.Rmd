---
title: "EWAS OF AUTOSOMAL SEX-DIFFERENCES"
subtitle: "A Meta-analysis in the Athero-Express"
author: "Sander W. van der Laan, PhD"
date: "2018-10-15"
mainfont: Arial
output:
  html_notebook:
...
#### Background
We performed an _epigenome-wide association study_ (EWAS) of autosomal sex-differences using data from the [Athero-Express Biobank Study](http://www.atheroexpress.nl). Previously we published an [EWAS on tobacco smoking](https://www.ahajournals.org/doi/10.1161/CIRCGEN.117.002030) based on plaque-derived _DNA methylation_ (DNAm) as measured using the [Illumina Human Methylation 450K BeadArray](https://www.ncbi.nlm.nih.gov/pubmed/22126295). 
Here we performed an EWAS on sample 'sex' with and without correction for testosterone/estradiol ratios (T/E2). We will use the [DNAmArray libary](https://github.com/molepi/DNAmArray) and associated [Workflow](https://molepi.github.io/DNAmArray_workflow/index.html) for this.  
...
#### Setting the stage
To load libraries we will use a function that will automatically check in both CRAN and Bioconductor. This is a function I found online from [Samir](http://stackoverflow.com/questions/4090169/elegant-way-to-check-for-missing-packages-and-install-them). 
...
<br />
Now we will load some packages; I like loading them at the beginning of the work, just to keep track of things.

_For survival analyses._
```{r eval = TRUE}
...
<br />
_For methylation/RNA data._
<br />
You are going to have to install `RMySQL` yourself via CRAN - doing this via `install.packages.auto` or the native `install.packages` did not work for me.
```{r eval = TRUE}
...
<br />
_The actual DNAmArray package._
...
We will also need a function to create Manhattans and QQ-plots; we used the [`qqman` library](https://cran.r-project.org/web/packages/qqman/) for this and edited it for our purpose.
...
<br />
Finally we will create a datestamp and define the _Utrecht Science Park Colour Scheme_.
```{r eval = TRUE}
...
<br/>
After setting the stage, let's turn a page and continue.
...
### Setup Analysis {.tabset .tabset-fade .tabset-pills}
#### Set locations
We can assess where we are using `
getwd()`, and set some locations. For reasons of privacy these are set machine dependent, locally and absolute.
...
<br/>
There are also some useful variables we can set and use downstream.
```{r eval = TRUE}
...
<br />
And we will also set some directories to output data.
```{r eval = TRUE}
...
#### Load data
We will load the two _Athero-Express Methylation Study_ datasets, details on the experimental setup can be found [elsewhere](https://www.ahajournals.org/doi/10.1161/CIRCGEN.117.002030).

<br/>
Loading B/Mvalues of plaque samples, dataset AEMS450K1 and AEMS450K2.
```{r eval = TRUE}
setwd(INP_loc)
list.files()
...
<br />
Sanity checks and plotting. Please note the unit and some details on the measurements of these sex hormones:
...
We will define the following colors in the next few graphs:
...
<br />
Creating interleaved histograms with added mean lines for each of the three hormones, and the testosterone/estradiol ratio.

<br />
_Testosterone ..._
```{r eval = TRUE, fig.align = 'center', fig.cap = 'Figure 1: Testosterone distributions.'}
library(ggplot2)
...
<br />
_Estradiol ..._
```{r eval = TRUE, fig.align = 'center', fig.cap = 'Figure 2: Estradiol distributions.'}
...
<br />
_SHBG ..._
```{r eval = TRUE, fig.align = 'center', fig.cap = 'Figure 3: SHBG distributions.'}
...
<br />
_Testosterone/Estradiol Ratio ..._
```{r eval = TRUE, fig.align = 'center', fig.cap = 'Figure 4: T/E2 distributions.'}
...
<br />
Let's plot these in a nice 2x2 figure.
```{r eval = TRUE, fig.align = 'center', fig.cap = 'Figure 5: Hormonal distributions.'}
library(gridExtra)

pdf(paste0(QC_loc,"/",Today,".aems450k1.",EWAS_trait_Hormones,".Hormones.pdf"),
...
<br />
Let's also do some sannity checking on the genome-wide methylation data. 

<br />
_AEMS450K1, in plaque..._
```{r eval = TRUE, fig.align = 'center', fig.cap = 'Figure 6: AEMS450K1 Methylation Density.'}
...
<br />
_AEMS450K2, in plaque..._
```{r eval = TRUE, fig.align = 'center', fig.cap = 'Figure 7: AEMS450K2 Methylation Density.'}
...
<br />
We will remove the `BvaluesQCIMP`-objects since we don't use these anymore.
```{r eval = TRUE}
rm(aems450k1.BvaluesQCIMPplaqueSE, aems450k2.BvaluesQCIMPplaqueSE)
```

### Meta-Analysis {.tabset .tabset-fade .tabset-pills}
To perform this meta-analysis we followed (parts of) the [DNAmArray Workflow](https://molepi.github.io/DNAmArray_workflow/06_EWAS.html) developed by the group of prof. dr. Bastiaan Heijmans.

#### Modeling and preparation
<br />
Setup the analysis and the model, first variable is the phenotype of interest. Please note:
By default, R will choose a reference level for factors based on [alphabetical order](https://www.bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html). 
Then, if you never tell the DESeq2 functions which level you want to compare against (_e.g._ which level represents the control group), the comparisons will be based on the alphabetical order of the levels. 
```{r eval = TRUE}
...
<br />
Next, we extract those samples having a complete set of covariates. Notice that we subset the SummarizedExperiment-object!
...
<br />
Remove probes on the X and Y chromosomes and probes containing SNPs or does mapping to multiple locations (ref: W. Zhou, P.W. Laird, and H. Shen. 2016. Nucleic Acids Res.).

<br />
First chromosome X and Y (remember: we are interested in _autosomal_ sex-differences)...
...
aems450k1.MvaluesQCplaqueClean_T_E2 <- aems450k1.MvaluesQCplaqueClean_T_E2[!(names(aems450k1.MvaluesQCplaqueClean_T_E2) %in% names(hm450.manifest.pop.GoNL)),]
```
...
* what is different in AEMS450K1...?
```{r eval = TRUE}
...
length(aems450k1.meta.difs) # 40,625
length(aems450k1.meta.difs_T_E2) # 40,625
```

* what is different in AEMS450K2...?
```{r eval = TRUE}
...
dim(aems450k1.MvaluesQCplaqueClean_T_E2)  # 431,834 CpGs, 113 samples
dim(aems450k2.MvaluesQCplaqueClean_T_E2)  # 431,834 CpGs, 93 samples
...
cat("\n\n - AEMS450K1 *with* hospital and T/E2 ratio.")
aems450k1.designph_T_E2 <- model.matrix(metadata(aems450k1.MvaluesQCplaqueClean_T_E2)$formula_T_E2, data = colData(aems450k1.MvaluesQCplaqueClean_T_E2))
aems450k1.dataph_T_E2 <- assays(aems450k1.MvaluesQCplaqueClean_T_E2)$data
aems450k1.fitph_T_E2 <- limma::lmFit(aems450k1.dataph_T_E2, aems450k1.designph_T_E2)
cat("  > calculate T-statistic = beta/standard error...")
aems450k1.effectsizeph_T_E2 <- aems450k1.fitph_T_E2$coefficients
aems450k1.SEph_T_E2 <- aems450k1.fitph_T_E2$stdev.unscaled * aems450k1.fitph_T_E2$sigma
aems450k1.tstatph_T_E2 <- aems450k1.effectsizeph_T_E2/aems450k1.SEph_T_E2
cat("  > calculate p-value from T-statistic, and adjust using FDR (Benjamin-Hochberg)...")
aems450k1.pvalph_T_E2 <- 2*pnorm(-abs(aems450k1.tstatph_T_E2[,2]))
aems450k1.padjph_T_E2 <- p.adjust(sort(aems450k1.pvalph_T_E2, decreasing = FALSE), method = "BH")
head(aems450k1.padjph_T_E2[aems450k1.padjph_T_E2 < 0.05])
# summary
summary(aems450k1.fitph_T_E2)
```

-----
<sup>&copy; 1979-2018 Sander W. van der Laan | s.w.vanderlaan[at]gmail.com | [swvanderlaan.github.io](swvanderlaan.github.io).</sup>
